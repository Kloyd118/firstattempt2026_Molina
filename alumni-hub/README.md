...
## ⚠️ AI Hallucinations & Manual Fixes
During the process, I had to manually intervene to fix the following AI errors:

1. **TypeScript Configuration Conflict:**
   - **The Error:** The AI suggested a manual `compilerOptions` list that broke Nuxt's auto-imports.
   - **The Fix:** I manually restored `"extends": "./.nuxt/tsconfig.json"` to ensure the PWA module and Nuxt 3 remained compatible.

2. **Environment Pathing (OneDrive Sync Error):**
   - **The Error:** Running the project inside a OneDrive-synced folder caused `ENOENT` errors during the PWA build process.
   - **The Fix:** I migrated the project to a local root directory to prevent file-locking issues.

3. **Manifest UI Warnings:**
   - **The Error:** The AI generated a manifest missing `purpose: 'any maskable'` and `screenshots`, causing Chrome DevTools warnings.
   - **The Fix:** I manually updated `nuxt.config.ts` with these properties to ensure a "Richer Install UI" and full PWA compliance.
